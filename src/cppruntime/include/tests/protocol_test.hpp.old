...
namespace pj_tests
{
    ...
    public:
        ...
        void run()
        {
            static struct request req{.number = 0, .amount = 420.69};
            data = req;
            switch(get_label())
            {
    ...
    public:
        ...
        L1:
            ...
            if(std::holds_alternative<struct request>(data))
            {
                std::cout << "reader received a request\n";
                req = std::get<struct request>(data);
                std::cout << req.number << ", " << req.amount << std::endl;
            }
            else if(std::holds_alternative<struct reply>(data))
            {
                std::cout << "reader received a reply\n";
                rep = std::get<struct reply>(data);
    ...
    public:
        ...
        void run()
        {
            ...
            pj_runtime::pj_one2one_channel<pj_runtime::pj_protocol<struct request, struct reply>> oto_ch;

            std::cout << "\n *** CREATING READER AND WRITER PROCESSES *** \n\n";
            auto* oto_w =
                new protocol_writer<pj_runtime::pj_protocol<struct request, struct reply>>(0, &oto_ch);
            auto* oto_r =
                new protocol_reader<pj_runtime::pj_protocol<struct request, struct reply>>(1, &oto_ch);

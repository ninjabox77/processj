...
public class CodeGenJava extends Visitor<Object> {
    ...
    private Compilation curCompilation = null;
    ...
    private String curParBlock = null;
    
    /** Currently executing protocol */
    private String curProtocol = null;
    ...
    public Object visitProcTypeDecl(ProcTypeDecl pd) {
        ...
        String prevProcName = curProcName;
        ...
        curProcName = (String) pd.name().visit(this);
        ...
        if ("Anonymous".equals(curProcName)) {
            ...
            stProcTypeDecl.add("parBlock", curParBlock);
            ...
            if (doesProcYield) {
                ...
                procName = Helper.makeVariableName(curProcName + createProcSignature(pd), 0, Tag.PROCEDURE_NAME);
                ...
                procName = Helper.makeVariableName(curProcName + createProcSignature(pd), 0, Tag.METHOD_NAME);
            ...
            if ("main".equals(curProcName) && pd.signature().equals(Tag.MAIN_NAME.toString())) {
                // Create an instance of a Java main method template
                ST stMain = stGroup.getInstanceOf("Main");
                stMain.add("class", curCompilation.fileNoExtension());
    ...
    public Object visitBinaryExpr(BinaryExpr be) {
        ...
        if ("instanceof".equals(op) && localToFields.containsKey(lhs)) {
            ...
            if (namedType.equals(PJProtocolCase.class.getSimpleName())) {
                stBinaryExpr = stGroup.getInstanceOf("RecordExtend");
                stBinaryExpr.add("name", lhs);
                stBinaryExpr.add("type", curProtocol);
                return stBinaryExpr.render();
            }
        }
        // -->
    ...
    public Object visitLocalDecl(LocalDecl ld) {
        ...
        if (expr == null) {
            if (!ld.type().isBarrierType() && (ld.type().isPrimitiveType() ||
                ...
                return null;                // The 'null' value is used to removed empty sequences in
                                            // the generated code
        }
    ...
    public Object visitInvocation(Invocation in) {
        ...
        if (curCompilation.fileName.equals(pd.myCompilation.fileName)) {
        ...
        if (Helper.doesProcYield(pd) && curParBlock == null) {
            return (new ParBlock(
        ...
        if (Helper.doesProcYield(pd)) {
            ...
            stInvocation.add("parBlock", curParBlock);
            // Add the barrier this procedure should resign from
            if (!barriers.isEmpty())
...
//        String guard = (String) stat.visit(this);
    ...
    private String createProcSignature(ProcTypeDecl pd) {
        ...
        for (ParamDecl param : pd.formalParams()) {
            ...
            else if (param.type().isRecordType()) // <Rn; where n is the name
                s = s.replace("<", "rc").replace(DELIMITER, "");
            else if (param.type().isProtocolType()) // <Pn; where n is the name
